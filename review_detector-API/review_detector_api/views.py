...
loaded_model = joblib.load(
    open(path_to_model+'review_detector_model.pkl', 'rb'))
...
def preprocessing(review_text, rating, review_helpful):
    ...
    ans = [rating, review_helpful, sentiment, subjectivity,
           word_count, noun[0], adjective[0], verb[0], adverb[0]]
    return ans
...
def detect_review(request):
    ...
    try:
        ...
        for rev_text, rating, review_useful in zip(rev_text_list, review_useful_list, rating_list):
            ...
            testing = pd.DataFrame(columns=['Rating',	'Review_helpful',	'Sentiment',	'Subjectivity',
                                   'Word_Count',	'Noun_Count',	'Adj_Count',	'Verb_Count',	'Adv_Count'])
